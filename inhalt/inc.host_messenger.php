    ...
    for ($k=1;$k<11;$k++) {
        if (($spieler_id_c[$k]>=1) and ($spieler_raus_c[$k]==0)) {
            ...
            if ((substr($optionen,1,1)=='1') and (strlen($jabberr)>=3)) {
                $hash=$spieler_hash[$k];
                ?>
                <iframe src="inc.host_messenger.php?fu=2&jab=<?php echo $jabberr?>&sname=<?php echo $spiel_name?>&hash=<?php echo $hash?>&k=<?php echo $k?>" style="border:0px;width:0px;height:0px;" scrolling="no" marginheight="0" marginwidth="0" frameborder="0"></iframe>
                <?php
            }
        }
    }
